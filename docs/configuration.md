...
| `metrics_bind_addr` | `host:port` | valid socket addr | Address the Prometheus `/metrics` endpoint binds to. Use `0.0.0.0:9000` inside docker compose so the `prometheus` container can scrape it; use `127.0.0.1:9000` for local-only exposure. |
