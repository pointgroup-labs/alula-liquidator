...
| `assets_to_hold` | map `strkey -> u16` (bps) | ≥ 1 entry, valid addresses, values 0–10000 and summing to exactly 10000 | Assets the keeper wants to keep on its balance, mapped to a target share of held value in basis points (10000 = 100%). **The asset with the largest share is the Balancer's swap target(for now)** — everything not in this map gets rebalanced into it. The liquidator also treats these as the candidate *source* assets for pre-swap liquidations (swaps a held asset into the repay asset). *Note: the distribution values are validated and parsed but not yet consumed by the Balancer.* |
