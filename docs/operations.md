...
**Is the bot making money?** *Expected profit per plan* shows the per-opportunity USD distribution at dispatch time (p50/p95). *Expected profit dispatched (USD/hour)* is the run-rate headline. Both are modelled-not-realised — they use `plan.net_profit_oracle` before tx submission, so submission failures and simulation drift between plan-build and on-chain execution will widen the realised-vs-expected gap. Cross-reference with *TX confirmation outcomes*: a sustained gap between "confirmed" and the dispatched plan rate is the realisation tax.
